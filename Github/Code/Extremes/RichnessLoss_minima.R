...
amf@sam_data$MAT_min<-meta_out$MAT_min
amf@sam_data$MAP_min<-meta_out$MAP_min
...
df_scaled <- data %>%
  dplyr::select(MAP_min, MAT_min, NP, CN, pH) %>%
...
#  "Climate_no_slope" = "Chao1 ~ MAP_min + MAT_min + (1 | Site_ID)",
#  "ClimateLanduse_add_no_slope" = "Chao1 ~ MAP_min + MAT_min + IsFarm + (1 | Site_ID)",
#  "ClimateLanduse_add_soil_no_slope" = "Chao1 ~ MAP_min + MAT_min + IsFarm + CN + NP + pH + (1 | Site_ID)",
#  "ClimateLanduse_int_no_slope" = "Chao1 ~ MAT_min * IsFarm + MAP_min * IsFarm + CN + NP + pH + (1 | Site_ID)",
...
#  "Climate_with_slope" = "Chao1 ~ MAP_min + MAT_min + (1 + IsFarm | Site_ID)",
#  "ClimateLanduse_add_with_slope" = "Chao1 ~ MAP_min + MAT_min + IsFarm + (1 + IsFarm | Site_ID)",
#  "ClimateLanduse_add_soil_with_slope" = "Chao1 ~ MAP_min + MAT_min + IsFarm + CN + NP + pH + (1 + IsFarm | Site_ID)",
  "ClimateLanduse_int_with_slope" = "Chao1 ~ MAT_min * IsFarm + MAP_min * IsFarm + CN + NP + pH + (1 + IsFarm | Site_ID)"
)
...
best_fit <- brm(
  Chao1 ~ MAT_min * IsFarm + MAP_min * IsFarm + CN + NP + pH + (1 + IsFarm | Site_ID),
  data = df_scaled,
  iter = 1000,
  warmup = 250,
  chains = 4,
...
effects <- conditional_effects(best_fit, effects = "MAT_min:IsFarm", prob = 0.95)
effects_df <- as.data.frame(effects$`MAT_min:IsFarm`)
MATxFarm_Plot <- ggplot(effects_df, aes(x = MAT_min, y = estimate__, color = IsFarm)) +
  stat_smooth(method = "lm", se = FALSE, size = 3) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.1) +
  theme_minimal()

effects <- conditional_effects(best_fit, effects = "MAP_min:IsFarm", prob = 0.95)
effects_df <- as.data.frame(effects$`MAP_min:IsFarm`)
MAPxFarm_Plot <- ggplot(effects_df, aes(x = MAP_min, y = estimate__, color = IsFarm)) +
  stat_smooth(method = "lm", se = FALSE, size = 3) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__), alpha = 0.1) +
  theme_minimal()
...
sd_mat <- 2.58     # Standard deviation for MAT_min (degrees Celsius)
sd_map <- 17.49    # Standard deviation for MAP_min (mm precipitation)

# Extract coefficients and uncertainties
coef_summary <- posterior_summary(best_fit)

# MAT_min results for IsFarm = FALSE (uncultivated)
mat_coef_farm_false <- coef_summary["b_MAT", "Estimate"]
mat_uncertainty_farm_false <- coef_summary["b_MAT", "Est.Error"]
best_fit
# Print MAT_min results for IsFarm = FALSE
print("Coefficient for MAT_min when IsFarm = FALSE (uncultivated):")
print(mat_coef_farm_false)
print("Uncertainty (Est.Error) for MAT_min when IsFarm = FALSE (uncultivated):")
print(mat_uncertainty_farm_false)

# MAT_min hypothesis testing
hypothesis_false <- hypothesis(best_fit, "MAT_min < 0")
hypothesis_true <- hypothesis(best_fit, "MAT_min:IsFarmTRUE > 0")

# Print hypothesis testing results for MAT_min
print("Hypothesis test for MAT_min when IsFarm = FALSE (uncultivated):")
print(hypothesis_false)
print("Hypothesis test for MAT_min when IsFarm = TRUE (farm):")
print(hypothesis_true)
hypothesis_gradients <- hypothesis(best_fit, "MAT_min:IsFarmTRUE < 2.27",alpha = 0.05) #test if climate gradients sig,NA is significant


# MAP_min results for IsFarm = FALSE (uncultivated)
map_coef_farm_false <- coef_summary["b_MAP_max", "Estimate"]
map_uncertainty_farm_false <- coef_summary["b_MAP", "Est.Error"]

# Print MAP_min results for IsFarm = FALSE
print("Coefficient for MAP_min when IsFarm = FALSE (uncultivated):")
print(map_coef_farm_false)
print("Uncertainty (Est.Error) for MAP_min when IsFarm = FALSE (uncultivated):")
print(map_uncertainty_farm_false)

# MAP_min hypothesis testing
hypothesis_map_false <- hypothesis(best_fit, "MAP_min > 0")
hypothesis_map_true <- hypothesis(best_fit, "IsFarmTRUE:MAP_min > 0")

# Print hypothesis testing results for MAP_min
print("Hypothesis test for MAP_min when IsFarm = FALSE (uncultivated):")
print(hypothesis_map_false)
print("Hypothesis test for MAP_min when IsFarm = TRUE (farm):")
print(hypothesis_map_true)
hypothesis_gradients <- hypothesis(best_fit, "IsFarmTRUE:MAP_min = -4.71") #test if climate gradients sig differ
