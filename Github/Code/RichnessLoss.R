...
data <- read.csv("/Users/justinstewart/Dropbox/Collaborations/Ecuador/Github/Data/EcuadorGEE.csv") %>%
  left_join(mapping, .) %>%
...
library(broom.mixed)   # for tidying brms/lme4 outputs
library(tibble)
library(dplyr)
library(purrr)

# Fit all models
fits <- map(model_list, ~ brm(.x, data = df_scaled, refresh = 0, sample_prior = "yes"))

# Extract parameter estimates for each model
model_summaries <- map2_dfr(
  fits,
  names(fits),
  ~ {
...
# Save to CSV
write.csv(model_summaries, "/Richness_Model_Estimates.cv", row.names = FALSE)
